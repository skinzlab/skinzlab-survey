...
// ─── Firebase Firestore REST API 설정 ─────────────────
const FIREBASE_API_KEY = "AIzaSyB8whBafl6c0HQtinOUh0ZgXHBUL9f1SuY";
const FIREBASE_PROJECT_ID = "skinzlab-msti";
const FIRESTORE_BASE = `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/(default)/documents`;

// Firestore REST API 헬퍼 함수
const firestoreAPI = {
  // 응답 저장 (responses 컬렉션에 새 문서 추가)
  async addResponse(answers) {
    const url = `${FIRESTORE_BASE}/responses?key=${FIREBASE_API_KEY}`;
    const body = {
      fields: {
        data: { stringValue: JSON.stringify(answers) },
        submitted_at: { integerValue: String(Date.now()) },
      },
    };
  ...
  // 전체 응답 읽기 (페이지네이션 포함, 최대 1000건)
  async getAllResponses() {
    const url = `${FIRESTORE_BASE}/responses?key=${FIREBASE_API_KEY}&pageSize=1000`;
    const res = await fetch(url);
    if (!res.ok) {
      const err = await res.json().catch(() => ({}));
  ...
  },
  {
    ...
    questions: [
      ...
      { id: "q_email", text: "이메일 주소를 남겨주시면 베타 오픈 시 가장 먼저 연락드릴게요! 📩", type: "email",
        note: "📌 베타 참여 의향자에게만 표시됨 (선택)", showIf: { qId: "q_beta", values: ["꼭 참여하고 싶다", "관심 있다 (정식 출시 시 사용 예정)"] } },
    ]
  }
];
...
export default function App() {
  const [view, setView] = useState("survey"); // "survey" | "done" | "results"
  ...
  const handleSubmit = async () => {
    ...
    try {
      await firestoreAPI.addResponse({ ...answers, submitted_at: Date.now() });

      // 제출 후 응답 수 가져오기
      const allResponses = await firestoreAPI.getAllResponses();
      setTotalResponses(allResponses.length);
      setView("done");
    } catch (e) {
      console.error("Firebase 제출 오류:", e);
  ...
  const loadResults = async () => {
    ...
    try {
      ...
      setResultsData({ count, agg, responses: allResponses });
    } catch(e) {
  ...
  if (view === "survey") return (
    <div style={{ fontFamily: "'Noto Sans KR', Apple SD Gothic Neo, sans-serif", background: PALETTE.cream, minHeight: "100vh" }}>
      ...
      {/* Section tabs */}
      <div style={{ display: "flex", overflowX: "auto", gap: 0, background: "#fff", borderBottom: "1px solid #e2e8f0" }}>
        {SECTIONS.map((s, i) => (
          <button key={s.id} onClick={() => setCurrentSection(i)}
            style={{ flex: 1, minWidth: 80, padding: "12px 8px", border: "none", cursor: "pointer", fontSize: 12, fontWeight: currentSection === i ? 700 : 400,
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "24px 16px 40px" }}>
              ...
              {q.type === "open" && (
                <textarea value={answers[q.id] || ""} onChange={e => setAnswers(a => ({ ...a, [q.id]: e.target.value }))}
                  ...
                  style={{ width: "100%", border: "1.5px solid #e2e8f0", borderRadius: 8, padding: "12px 14px",
                    ...
                    background: "#fafafa", outline: "none", color: "#374151", boxSizing: "border-box" }} />
              )}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      style={{ width: "100%", border: `1.5px solid ${!isValid ? "#f87171" : emailVal ? "#22c55e" : "#e2e8f0"}`,
                        ...
                        background: "#fafafa", outline: "none", color: "#374151", transition: "border .2s", boxSizing: "border-box" }}
                    />
        ...
        <div style={{ textAlign: "center", marginTop: 16 }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 11, color: PALETTE.muted }}>
            <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#22c55e" }} />
            Firebase Firestore 연결됨 · {FIREBASE_PROJECT_ID}
          </div>
        </div>
    ...
    <div style={{ fontFamily: "inherit", background: PALETTE.cream, minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ background: "#fff", borderRadius: 20, padding: "48px 36px", textAlign: "center", maxWidth: 480, boxShadow: "0 8px 32px rgba(26,39,68,.1)" }}>
        ...
        <div style={{ background: "#eff6ff", borderRadius: 12, padding: "16px 20px", marginBottom: 28, textAlign: "left" }}>
          ...
          <div style={{ fontSize: 12, color: "#3b82f6" }}>Firebase Firestore에 실시간 저장됨</div>
        </div>
  ...
  if (view === "results") return (
    <div style={{ fontFamily: "inherit", background: PALETTE.cream, minHeight: "100vh" }}>
      <div style={{ background: `linear-gradient(135deg, ${PALETTE.navy}, #2a3f7a)`, padding: "28px 24px", textAlign: "center" }}>
        ...
        <p style={{ color: "rgba(255,255,255,.7)", fontSize: 13 }}>SKINCODE 사전 설문 · Firebase Firestore</p>
        {resultsData && (
      ...
      <div style={{ maxWidth: 720, margin: "0 auto", padding: "24px 16px 60px" }}>
        ...
        {firebaseError && (
          <div style={{ background: "#fef2f2", border: "1px solid #fecaca", borderRadius: 12, padding: "16px 20px", marginBottom: 16 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#dc2626", marginBottom: 6 }}>⚠️ Firebase 연결 오류</div>
            <div style={{ fontSize: 12, color: "#991b1b", marginBottom: 8 }}>{firebaseError}</div>
            <div style={{ fontSize: 11, color: "#b91c1c" }}>
              Firestore 보안 규칙에서 responses 컬렉션의 read/write를 허용해 주세요.
            </div>
          </div>
        )}

        {loadingResults ? (
          <div style={{ textAlign: "center", padding: 60, color: PALETTE.muted, fontSize: 15 }}>Firebase에서 데이터 불러오는 중...</div>
            ...
            {SECTIONS.map(sec => (
              <div key={sec.id}>
                ...
                {sec.questions.filter(q => q.type !== "open" && q.type !== "email").map((q, qi) => {
                  ...
                  return (
                    <div key={q.id} style={{ background: "#fff", borderRadius: 14, padding: "20px 20px", marginBottom: 12, border: "1px solid #e2e8f0" }}>
